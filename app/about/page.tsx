...
const TeamMember = ({ name, role, image }: { name: string, role: string, image?: string }) => (
    <div className="flex-1 h-fit flex flex-col gap-6 items-center md:items-start ">
        ...
        <div className="flex flex-col max-md:text-center gap-1 px-4">
            ...
            <p className="text-primary-700 font-neulisneue text-xl font-medium">{role}</p>
        </div>
    </div>
)
...
export default function About() {
    ...
    return (
        <main className="grid grid-cols-4 md:grid-cols-12 gap-24 min-h-screen bg-white font-neulissans tracking-tight py-[5.5rem] pb-0">
            <section className="col-span-full relative flex flex-col w-full px-2 md:px-4 z-10 ">
                <div className="flex w-full justify-between bg-primary-100 border border-primary-900/15 rounded-[15px] h-[60vh] overflow-hidden">
                    <div className="flex h-full items-end p-10 md:p-12 md:max-w-2xl ">
                        <h2 className=" text-5xl font-normal font-neulisneue tracking-tight leading-[3.2rem]">We engineer smarter water solutions for a <span className="font-medium italic">sustainable urban future.</span></h2>
                    </div>
                    <div className="flex items-center">
                        <img src={hero.src} className="h-[calc(100%+1rem)]" />
                    </div>
                </div>
            </section >
                ...
                <div className="col-span-full flex flex-col gap-12">
                    ...
                    <div className="flex gap-4 ">
                        <Button
                            ...
                            className={`font-neulisneue font-medium text-2xl tracking-tight w-fit transition-all duration-200 border-0 ${activeTab === 'mission' ? 'bg-primary-400' : 'opacity-40'}`}
                        ...
                        <Button
                            ...
                            className={`font-neulisneue font-medium text-2xl tracking-tight w-fit transition-all duration-200 border-0 ${activeTab === 'vision' ? 'bg-primary-400' : 'opacity-40'}`}
                    ...
                    <AnimatePresence mode="wait">
                        <motion.div key={activeTab} initial={{ opacity: 0, translateY: 20 }} animate={{ opacity: 100, translateY: 0 }} className="min-h-[120px] transition-all duration-300 ease-in-out flex gap-8">
            ...
            <section className="col-span-full flex flex-col gap-12 rounded-[45px_45px_0px_0px] bg-primary-100 border border-primary-900/15 ">
                <div className="grid grid-cols-4 md:grid-cols-12  gap-20 py-24">
                    <div className="col-span-full md:col-span-10 md:col-start-2 flex gap-16 py-8">
                        <h2 className="text-5xl tracking-tight leading-[3rem] flex-1">The team behind the scenes</h2>
                        <p className="text-xl text-black/70 flex-[2]">
                    ...
                    <div className="col-span-full md:col-span-10 md:col-start-2 grid grid-cols-3 gap-12 max-md:px-8">
                        <TeamMember
                            name="Vilas Gaikwad"
                            role="Director"
                        />
