  ...
  return (
    <section className="col-span-full grid grid-cols-12 gap-16 py-16 mt-8 bg-primary-100" id="solutions">
      <div className="col-span-10 col-start-2">
        <h2 className="text-6xl tracking-tighter font-medium text-black">Solutions</h2>
      </div>

      <div className="col-span-full grid grid-row-3 space-y-4 px-14">
        {solutions.map((solution, index) => {
          ...
          return (
            <div
              ...
              key={index}
              className={`relative flex gap-20 transition-all items-center justify-between duration-700 ease-out border-2 overflow-hidden  cursor-pointer ${isOpen
                ? "border-primary-900/40 bg-primary-200 rounded-[75px] pl-12"
                : "border-primary-900/15 bg-white rounded-[30px] px-10 py-8"
                }`}
            >
              <div className={`flex items-start gap-8 transition-all duration-700 ease-out ${isOpen ? "py-12 pt-16" : "p-0"}`}>
              ...
              <div className={`flex rounded-3xl w-full h-full overflow-hidden bg-primary-900 transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] ${isOpen ? "opacity-100" : "opacity-0"}`}>
              </div>
